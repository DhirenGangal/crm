...
class Admin extends MY_Controller
{
    ...
    public function child_sub_products($act = '', $str = '')
    {
        ...
        if ($act == "add") {
            if (!empty($_POST['product_name'])) {
                $pdata['parent_id'] = !empty($_POST['sub_product_id']) ? trim($_POST['sub_product_id']) : '';
                ...
                $last_id = $this->dbapi->addProduct($pdata);
                $folder = 'data/child-sub-products/';
                if (!empty($_FILES['product_logo']['name'])) {
                    $img = imgUpload("product_logo", $folder, slugify($pdata['product_name']) . $last_id);
                    $this->dbapi->updateProduct(["product_logo" => $img], $last_id);
                }
            }
            if (!empty($last_id)) {
                ...
                $last_id1 = $this->dbapi->addChildProductInfo($data);
                $folder = 'data/datasheets/';
                if (!empty($_FILES['datasheet']['name'])) {
                    $img = imgUpload("datasheet", $folder, slugify($pdata['product_name']) . $last_id);
                    $this->dbapi->updateChildProductInfo(["datasheet" => $img], $last_id);
                }
            }
    ...
    {
        $this->header_data['title'] = "Tickets";
        $data = array();
        $this->_admin('tickets', $data);
